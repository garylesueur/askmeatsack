# Questionnaire

**askmeatsack.com** — an agent asks a human questions. The tool is named askmeatsack.com. Create returns a link for the conversation or for an unattended job; the calling agent delivers it.

| Spec | Covers | Status |
| --- | --- | --- |
| [Answering a questionnaire](./sessions/answering.md) | Create, inline link, unattended one-per-person send, skill wait, one question at a time, evidence rail, review, autosave, text, choice, items, fields, files (choose, drop, or take a photo), recommended, opened, submit, cancel, expiry, JSON download, wait, callback, both doorways. Sketch questions, sections, welcome, named sources, and richer device capture are specified as future. | partial |

Read [answering](./sessions/answering.md) before changing how sessions are created, delivered, answered, waited on, or cancelled.
